...
public class DlReader {
    private static final String DL_READER_PACKAGE_NAME = "net.dlogic.ufr";
    ...
    static private byte[] AoAReadBuffer = new byte[MAX_BYTES]; // circular buffer
    static private int totalBytes;
    static private int writeIndex;
    static private int readIndex;
    ...
    private synchronized boolean updateContext(Context context) {
    ...
    private boolean openAoAEndpoints(UsbAccessory accessory) {
        ...
        if (fd != null) {
            ...
            //Toast.makeText(mContext, "Otvoriga ja pa...", Toast.LENGTH_SHORT).show();
        } else {
            return false;
        }
        ...
        public static void portWrite(byte[] buffer, int buffer_size) throws DlReaderException {
            if (ft_device != null) {
                ...
                try {
                    ...
                    throw new DlReaderException("UFR_COMMUNICATION_BREAK", DlErrorCodes.AOA_WRITE_ERROR);
                }
            }
        }
    ...
    private void setAoAConfig() throws DlReaderException {
        byte[] configdata = new byte[8];
        int rs232_baud_rate = 115200;
